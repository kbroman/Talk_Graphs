...
![piechart](Figs/fig2b.png)

Note that even worse that piecharts are donut plots.

![donut](http://upload.wikimedia.org/wikipedia/commons/thumb/1/11/Donut-Chart.svg/360px-Donut-Chart.svg.png)
...
## Example 2: Barplots as data summaries

While barplots are useful for showing percentages, they are commonly used to display data from two groups begin compared badly is to create two barplots with height equal to the mean and add the standard error as an antenna at the top. This plot gives us no information about the data. 

![threeddbarplot](Figs/fig1c.png)

Much more informative is to summarizing wit boxplot. If the number of points is small enough, we might as well add them.
When the number of points is too large for us to see them, a boxplot is preferable.
...
Note how much more we see here: the center, spread, range and the points themselves while in the barplot we only see the mean and the SE which has more to do with sample size than the spread of the data.

This problem is magnified when our data has outlier or very large tails. Note that from this plot there appears to be very large and consistent difference between the two groups:

![baroplotnolog](Figs/fig3c.png)

A quick look at the data demonstrates that this difference is driven by just two points. A version showing the data in the log-scale is much more informative.
...
The purpose of many statistical analyses is to determine relationships between variables. Sample correlations are typically reported and sometimes plots are displayed to show this. However, showing just the regression line tells is one way to display you data baldy as it hides the scatter. Surprisingly plots such as the following are commonly seen:
...
# High correlation does imply replication
...
In the plot on the left we see the original data which shows very high correlation. But the data follows a distribution with a very fat tails. Note that 99% of the data is below the green line. The plot on the right is in the log scale. 
...
Now the correlation is very close to 1 in both cases. Does this mean these data are perfectly reproduced? What we are really interested in is the difference so we should plot that. In this plot we plot the difference (in the log scale) versus the average
...
We now see that the typical difference in the log (base 2) scale between two replicated measures is about 1. This means that when measurements should be the same we will, on average, observe 2 fold difference. We can now compare this variability to the differences we want to detect and decide if this technology is precise enough for our purposes.


# Barpots for paired data

A common task in data analysis is the comparison of two groups. When these data are paired, for example outcomes before and after a treatment, and is small enough, an unfortunate display that is used is the barplot with two colors:
![pairs](Figs/fig6r_e.png)

There are various ways to show this data an illustrate there is an increase. One is to simply make a scatterplot and show the identity line. Another alternative is plot the differences against the Before values.
```{r}
set.seed(12201970)
before <- runif(6, 5, 8)
